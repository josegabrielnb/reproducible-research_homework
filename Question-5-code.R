...
Virus_data<-read.csv("question-5-data/Cui_etal2014.csv")#read in the data in csv form
nrow(Virus_data)#number of rows
ncol(Virus_data)#number of columns
...
model1<-lm(log(Virion.volume..nm.nm.nm.) ~ log(Genome.length..kb.), Virus_data)#turns into linear model
summary(model1)
