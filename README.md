...
b) perform a log transformation on the relationship, to form lnV = lnβ + αlnL  
...
e) V = β*((L)^α), where β = exp(7.0748), α = 1.5152, volume is measured in nm^3, length is measured in kb
...
issues with using github..
- ideas: not accessible if it has premium features?
- see - can version history increase reproducibility
